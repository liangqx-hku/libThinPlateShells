...
bool loadElastic(const std::string& path, ElasticSetup& setup, ElasticState& state)
{
    ...
    else
    {
        ...
        for (int i = 0; i < nclamped; i++)
        {
            ...
            if (!ss)
            {
                std::cout << "-using rest position for clamped dofs " << dofId << std::endl;
                int vid = dofId / 3;
                int coordId = dofId % 3;
                setup.clampedDOFs[dofId] = state.curPos(vid, coordId);
            }
            else
            {
                setup.clampedDOFs[dofId] = std::stod(dofVal);
            }
        }
    ...
    // point forces
    setup.pointForces.clear();
    if (!jval.contains(std::string_view{ "point_Forces" }))
    {
        std::cout << "missing point forces path." << std::endl;
    }
    else
    {
        setup.pointForcesPath = jval["point_Forces"];
        std::ifstream ifs(filePathPrefix + setup.pointForcesPath);
        if (!ifs)
        {
            std::cout << "Missing " << setup.pointForcesPath << std::endl;
            return false;
        }
        int nPointForces;
        ifs >> nPointForces;
        ...
        if (!ifs)
        {
            std::cout << "Error in " << setup.pointForcesPath << std::endl;
            return false;
        }
        ifs.ignore(std::numeric_limits<int>::max(), '\n');
        std::cout << "num of point forces: " << nPointForces << std::endl;
        for (int i = 0; i < nPointForces; i++)
        {
            std::string line;
            std::getline(ifs, line);
            std::stringstream ss(line);
            ...
            if (x[0] != '#')
            {
                setup.pointForces[vid] = std::stod(x);
            }            
        }
    ...
    if (!jval.contains(std::string_view{ "output_mesh" }))
    {
        std::cout << "missing output_mesh path." << std::endl;
    }
    else
    {
        setup.outMeshPath = jval["output_mesh"];
    }
